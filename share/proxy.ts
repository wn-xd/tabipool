 ...
 */
function addKeys(rawKeys: string[], credits: Map<string, number>): string[] {
  const added: string[] = [];
  for (const key of rawKeys) {
    ...
    keys.push({
      ...
      key,
      tail: key.slice(-5),
      initialCents: credits.get(key) ?? DEFAULT_INITIAL_CENTS,
      spendCents: 0,
      sincePoll: 0,
      requests: 0,
      failures: 0,
...
async function loadKeys() {
  const f = Bun.file(KEYS_FILE);
  if (!(await f.exists())) {
    console.error(`fatal: ${KEYS_FILE} not found`);
    process.exit(1);
  }
  ...
  addKeys(raw, await loadInitialCredits());
  if (!keys.length) {
    console.error(`fatal: no keys in ${KEYS_FILE}`);
    process.exit(1);
  }
}

/** Appends new keys to keys.txt so a restart keeps them. */
async function persistKeysFile(newKeys: string[]) {
  const f = Bun.file(KEYS_FILE);
  const prev = (await f.exists()) ? await f.text() : "";
  const sep = prev.length && !prev.endsWith("\n") ? "\n" : "";
  await Bun.write(KEYS_FILE, prev + sep + newKeys.join("\n") + "\n");
}

/** Rewrites keys.txt from the live pool, used after a removal. */
async function rewriteKeysFile() {
  await Bun.write(KEYS_FILE, keys.map((k) => k.key).join("\n") + "\n");
}
...
function available(now: number) {
  return keys.filter((k) => !k.dead && k.coolUntil <= now);
}
 ...
 */
function pickKey(exclude: Set<number>, now: number): Key | null {
  let pool = available(now).filter((k) => !exclude.has(k.id));
...
async function pollBalances() {
  // One GET per key does double duty: authoritative spend, plus liveness. GETs are
  // immune to the POST-only origin blip, and this path is not IP-rate-limited.
  const batch = 8;
  for (let i = 0; i < keys.length; i += batch) {
    await Promise.all(
      keys.slice(i, i + batch).map(async (k) => {
        try {
          const r = await fetch(`${UPSTREAM}/v1/dashboard/billing/usage`, {
...
async function handleProxy(req: Request): Promise<Response> {
  ...
  for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
    const key = pickKey(tried, Date.now());
    if (!key) {
  ...
  srv.timeout(req, 0);
  {
    ...
    if (url.pathname === "/_keys/add" && req.method === "POST") {
      ...
      try {
        ...
        return jsonError(400, "body must be JSON: {\"keys\": [\"sk-...\"]}");
      }
      ...
      // Validate against upstream before admitting: a key that cannot authenticate
      // would otherwise sit in the pool absorbing requests and failing them.
      const checked = await Promise.all(
        submitted.map(async (key) => {
          ...
          try {
            ...
            return { key, ok: false, why: `upstream rejected with ${r.status}` };
          } catch (e) {
      ...
      const added = good.length ? addKeys(good, await loadInitialCredits()) : [];
      if (added.length) {
        await persistKeysFile(good);
        // Immediately fetch real spend so the new keys rank correctly for selection.
        await pollBalances();
        console.log(`[pool] added ${added.length} key(s): ${added.map((t) => "..." + t).join(", ")}`);
      }
